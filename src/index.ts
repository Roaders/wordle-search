...
import { argsOptions, wordleSearchArgConfig } from "./constants";
...
async function wordleSearch(){
    const args = parse<IWordleSearchArgs>(wordleSearchArgConfig, argsOptions);
    ...
    const knownCharacters = args.known != null ? Array.from(args.known.toLowerCase()).map(character => characterRegExp.test(character) ? character : undefined) : undefined;



    const matchedWords = words.filter(word => filterWord(word, {length: args.length, excludeRegexp, includeCharacters, knownCharacters}))
...
function filterWord(word: string,  options: FilterOptions): boolean{
    ...
    if(options.includeCharacters != null && !options.includeCharacters.every(character => word.indexOf(character) >= 0) ){
        return false;
    }
    if(options.excludeRegexp != null && !options.excludeRegexp.test(word)){
        return false;
    }
    if(options.knownCharacters != null && !options.knownCharacters.every((character, index) => character == null || word.charAt(index) === character)){
        return false;
    }
